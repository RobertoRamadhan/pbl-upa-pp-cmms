...
export async function GET(request: Request) {
  try {
    ...
    const onTimeCompletions = assignments.filter((assignment) => {
      ...
      const timeDiff =
        (completionTime.getTime() - assignedTime.getTime()) / (1000 * 60 * 60);
    ...
    const performance = {
      ...
      onTimePercentage:
        totalAssignments > 0 ? (onTimeCompletions / totalAssignments) * 100 : 0,
      averageCompletionTime:
        assignments.reduce((acc, curr) => {
          if (curr.endTime && curr.assignedAt) {
            return (
              acc +
              (curr.endTime.getTime() - curr.assignedAt.getTime()) /
                (1000 * 60 * 60)
            );
          }
          return acc;
